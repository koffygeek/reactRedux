...
import { decrementFruit, incrementFruit ,deleteFruit} from "../redux/action";

const Cart = ({fruit,user,incrementItem,decrementItem,removeItem}) => { 
    ...
    <div className="flex justify-center">
        <div className=" m-5 p-2.5 w-96 border-green-400 rounded bg-gray-600">
            <h1>My Cart</h1>
            <button className="bg-red-600 w-4" onClick={()=>decrementIt(item.id)}>-</button>
                <div className="flex-auto flex-col">
                    {fruit.map((item)=>(
                        <div key={item.id} className="w-52 mt-1 p-1 border-orange-400 bg-red-400 rounded">
                            ...
                            <button className="bg-red-700 mt-3" onClick={()=>removeItem(item.id)}>x</button>
                        </div>
                    ))}
                </div>
